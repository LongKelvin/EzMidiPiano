...
import com.example.chordec.chordec.Helper.Key;
...
//        Log.i("CHORDVIEW_ WHITEK_L: ", String.valueOf(whites_key.size()));
    ...
    public void setKey(int noteNumber, boolean isNoteOn) {
        ...
        if (indexOfKey.first != -1) {
            ...
            handleKeyNotFound(noteNumber,true);
            return;
    ...
    private void handleKeyNotFound(int note, boolean isNoteOn){
        int temp = -1;
        if(note<36)
            temp = note+12;
        else if (note>=47)
        ...
        Pair<Integer, Integer> indexOfKey = getKeyByNoteNumber(temp);

        if (indexOfKey.first != -1) {
            int index = indexOfKey.first;
            if (indexOfKey.second == 0) {
                whites_key.get(index).isNoteOn = isNoteOn;
                Log.i("SET KEY ON", "WHITE_KEY_ " + temp);
            } else {
                blacks_key.get(index).isNoteOn = isNoteOn;
                Log.i("SET KEY ON", "BLACK_KEY_ " + temp);
            }
        } else {
            Log.e("SET KEY ON", "NOTE NOT FOUND ::  " + temp);
