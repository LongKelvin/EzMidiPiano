...
package com.kelvin.midi.midilib.event.meta;
    ...
    public static MetaEvent parseMetaEvent(long tick, long delta, InputStream in) throws IOException
    {
        ...
        if(isText)
        {
            ...
            switch(eventData.type)
            {
                ...
                case TRACK_NAME:
                    return new com.kelvin.midi.midilib.event.meta.TrackName(tick, delta, text);
                case INSTRUMENT_NAME:
                ...
                case SEQUENCER_SPECIFIC:
                    return new com.kelvin.midi.midilib.event.meta.SequencerSpecificEvent(tick, delta, eventData.data);
                default:
        ...
        {
            case SEQUENCE_NUMBER:
                return com.kelvin.midi.midilib.event.meta.SequenceNumber.parseSequenceNumber(tick, delta, eventData);
            case MIDI_CHANNEL_PREFIX:
                return com.kelvin.midi.midilib.event.meta.MidiChannelPrefix.parseMidiChannelPrefix(tick, delta, eventData);
            case END_OF_TRACK:
                return new EndOfTrack(tick, delta);
            case TEMPO:
                return com.kelvin.midi.midilib.event.meta.Tempo.parseTempo(tick, delta, eventData);
            case SMPTE_OFFSET:
                return com.kelvin.midi.midilib.event.meta.SmpteOffset.parseSmpteOffset(tick, delta, eventData);
            case TIME_SIGNATURE:
                return com.kelvin.midi.midilib.event.meta.TimeSignature.parseTimeSignature(tick, delta, eventData);
            case KEY_SIGNATURE:
