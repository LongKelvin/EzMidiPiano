...
public class SheetMusicActivity extends MidiHandlingActivity {
    ...
    public boolean drawerItemClickListener(IDrawerItem item) {
        ...
        if (identifier == R.id.song_settings) {
            changeSettings();
            drawer.closeDrawer();
        } else if (identifier == R.id.save_images) {
            showSaveImagesDialog();
            drawer.closeDrawer();
        } else if (identifier == ID_LOOP_START) {// Note that we display the measure numbers starting at 1,
        ...
        } else if (identifier == ID_LOOP_END) {
            AlertDialog alertDialog;
            String[] items;
            AlertDialog.Builder builder;// Note that we display the measure numbers starting at 1,
